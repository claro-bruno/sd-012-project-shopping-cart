...
const updateStorage = () => {
  ...
  if (localStorage.carrinho) {
    localStorage.carrinho = cart.innerHTML;
  } else {
    localStorage.setItem('carrinho', cart.innerHTML);
  }
};
...
const loadOff = async () => { 
  const load = document.querySelectorAll('.loading')[0];
  load.remove();
};
...
const clearBtn = () => {
  const btn = document.querySelectorAll('.empty-cart')[0];
  const clearFunc = () => {
